...
export function Melds({ melds, goldTileType, size = 'sm' }: { melds: MeldView[]; goldTileType: TileType; size?: TileSize }) {
...
/** Bonus tiles (winds/dragons drawn during play) — small label row. */
export function BonusTiles({
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {tiles.map((t, i) => (
        <Tile key={`${t}-${i}`} tile={t} size={size} isGold={goldTileType ? isGoldTile(t, goldTileType) : false} />
      ))}
    </div>
  );
}
