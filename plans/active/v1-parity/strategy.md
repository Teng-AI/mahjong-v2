...
5. (Added 2026-07-22 during M1; RE-RULED BY TENG 2026-07-22.) Gold wildcard chow placement: v1's win checker never let a gold stand in for a tile BELOW the lowest real tile of a run (holding 8+9, a gold could not complete 7-8-9), an artifact of its upward-only scan. Teng ruled for the rules doc: golds substitute for ANY tile in a run. v2 deliberately diverges from v1 code here; pinned by engine/__tests__/parity-edges.test.ts.
