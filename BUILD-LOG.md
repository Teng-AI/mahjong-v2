...
- Prod shipped same session: cleared spike rows on prod (convex import --replace), pushed to main (CI deployed functions, run 29933016015 green), vercel prod deploy. Gotcha: the M0-era service worker served a stale cached index (blank page); cleared SW + caches and the new bundle loaded. Vercel's git integration also auto-deploys on push, so manual deploy raced it (same commit, harmless)
- Prod smoke passed: Quick Play on https://mahjong-v2.vercel.app against jovial-sparrow-812; deal correct (wall 33 = 46 - 13 bonus replacements), my discard opened a calling phase, Bot-M1 pung'd it live and played on
- Next: M2 gate on a real iPhone (Quick Play vs 3 bots, screen locked through a calling phase and a bot turn). If the phone has the M0 PWA installed and shows a blank page, remove and re-add it (stale SW). Gate 7 usage glance while a game is live
